...
exports.deleteextraCategory = async (req, res) => {
    try {
        ...
        let p = await Product.deleteMany({ extracategoryname: extracategory._id });
        console.log(p)
...
exports.getAllExctraCategories = async (req, res) => {
    try {
        let extracategories = await Extracategory.find({ subcategoryname: req.query.categoryId });
        return res.json({ extracategories, message: "All SubCategory Fetched!!!" });
    } catch (error) {
        console.log(error);
        req.flash("error", "Server Error");
        return res.redirect("/");
